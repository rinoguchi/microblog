...
func createMapperFile(outputFilePath string, def utils.StructDef) error {
	...
	t := template.Must(template.ParseFiles("./model.tmpl"))
...
func main() {
	fmt.Println("usecase model generation started.")

	inputFilePaths, err := utils.ListUpFilePaths("../../../adapters/controllers/models", "models\\.gen\\.go")
	...
	for _, def := range defs {
		...
		outputFilePath := "../" + utils.ToSnakeCase(def.Name) + ".gen.edt.go"
	...
	fmt.Println("usecase model generation finished.")
}
